...
License: **AGPL-3.0** · 快照版本: **v3.38.0-audit**
...
| `core/core-ipc` | Engine↔Vault 签名回调契约（回调签名规则、错误码、防篡改；钱包密钥初始化/交易签名/双账户足额校验/账本对账摘要/交接承接请求契约） | 1 套 |
| `core/core-wallet` | 本地签名账本：交易模型与规范化序列化、域分离签名（SPARK-WALLET-TX-V1）、append-only 哈希链、双账户余额推导（custody/margin）、全链验签（重放/回退/断链检出）、钱包交接协议（HANDOVER 终结交易 + 交接证书 + 承接 GENESIS） | 1 套 |
...
- **本地钱包**：余额不是存储的数字，而是 append-only 签名交易历史的
  ...
  上服务端，也不进备份文件
- **双账户钱包（v3.38）**：托管（custody，储蓄层）与可用（margin，
  资金池）分离——托管层由密钥库侧权威账本足额校验（防盲签/防超支），
  可用层承载日常计费；高水位序号挡旧备份重放
- **钱包交接（v3.38）**：换机迁移零私钥拷贝——旧密钥签署 HANDOVER
  终结交易移交全部托管余额（此后旧链不可再续签），新机凭交接证书
  验证旧密钥签名后以 GENESIS 承接；交接证书防调包（新公钥绑定于
  旧密钥签名之内）
